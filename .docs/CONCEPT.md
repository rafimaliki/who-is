...
```mermaid
flowchart TD
    A[Landing page: name + filters] --> B[Broad search<br/>SearXNG + SerpAPI fallback]
...
- Free-tier-first stack, deliberately: SearXNG (self-hosted) for search, OpenRouter (cloud) for the LLM — no paid API required to run the POC end to end. SerpAPI is an optional paid search fallback. See [LLM_PIPELINE.md](./LLM_PIPELINE.md) for why each was picked over the alternatives.
